    ...
    return (
        ...
        >
            <Card className={`border-border/50 shadow-soft transition-all ${
                coupon.is_redeemed ? 'opacity-60' : isRedeemable ? 'hover:shadow-card hover:scale-[1.02]' : ''
            }`}>
...
export const LoveCoupons = () => {
    ...
    const [newCoupon, setNewCoupon] = useState({ title: '', description: '', emoji: '🎟️' });
    ...
    const myCoupons = coupons.filter(c => c.created_by !== user?.id && !c.is_redeemed);
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                ...
                <DialogContent>
                    ...
                    <div className="space-y-4 pt-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {templates.map((t, i) => (
                                    <Button
                                        ...
                                        size="sm"
                                        className="rounded-full"
                                        onClick={() => setNewCoupon({ ...newCoupon, title: t.title, emoji: t.emoji })}
                                    >
                                        {t.emoji} {t.title}
                                    </Button>
                                ))}
                        ...
                        <Button
                            onClick={() => createCoupon(newCoupon.title, newCoupon.description, newCoupon.emoji)}
                            disabled={!newCoupon.title.trim() || creating}
                            className="w-full rounded-full"
                        >
                            {creating ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Gift className="w-4 h-4 mr-2" />}
                            Create Coupon
                        </Button>
                    </div>
                </DialogContent>
                    ...
                    {myCoupons.length === 0 ? (
                        <Card className="border-border/50">
                            <CardContent className="p-8 text-center">
                                ...
                                <p className="text-muted-foreground">
                                    No coupons yet! Ask {user?.partner_name?.split(' ')[0]} for some treats 💝
                                </p>
                            </CardContent>
                        </Card>
                    ) : (
